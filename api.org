...
Abstractly, we're given data in json-ish format, and want to render it as a pretty-looking html table.

They have an ajax thing for this.


So basic idea: A flask/jinja HTML template. In order to html plot we secretly do two things:
We write json-ified data to data.json
Inside the HTML file, we basically do ~data=$.get('data.json', 'json')~ or whatever, and then plug that into a datatable that's appropriately configured.

TODO:
- Figure out the json file -> js object jquery pipeline
- Talk to Stephen about what properties the datatable should have aesthetically. This'll define the render functions for each column.
** Pipeline
$.get()
* Tabling - plan
- Modify webapp.py so we can either send a file or render a template intelligently
- Make the zcat_to_html function render/populate a jinja template. Pass in the path to the json data, possibly something like filters/conf info
* Dockering
- Short-term goal: Have something running in docker that can be demo'd
** Thinking out loud
** Storage/dirs/mechanisms we need
- Mount the desi stuff
- Read a config file (mounted)
- Store a crontab
- Cron job infrastructure
- Internal volumes: Cache
- Internal volumes: Store templates and stuff. More generally, a source repo. This is free
*** Dockerfile/Launching
- Install dependencies
- Including Stephen's trickery for installing desi stuff
- Patch/hackery for desi/bokeh templates
- Create volumes for the cache, and anything we ever write to.
- Crontab setup. So then all the cache-cleaning happens internally, in principle.
- So the entrypoint is "run cli.py with some user-specified args tacked on"
-

Benefit of using volumes: If one instance is running the server and the second does a cache-clean, it's fine! Performance is terrible, but it works as intended.
Realisation: I'm stupid, since if the cache_dir is a volume, we could have just hardcoded it the whole time. Ugh.
...
* Documentation TODOs
- Update endpoint userdocs, including docs on filters
- Docs on config file
- Docs on docker setup
- Reference to datatables+jinja
- Devdocs: Weird stuff like specifying the flask host
- The different kinds of cache cleaning
* Priority
- Doc
- Plot endpoints in the docker setup
- Test filter stuff
-
* Rebuild
- Redrock library and the redrock-templates directory. Kind of in progress

- Specify file format. DONE
- Table aesthetics: Include pretty-printed request parameters as well as request time
- Run more in-depth testing on filters+advanced features.
- TODO: Testing for post requests

What do filters look like?
PROGRAM=dark and stuff like that
* TODO
# - Set up a new desi-api venv, with everything from main, try to reproduce the keyerror QSO problem.
# - Keep notes on how the env was constructed.
- Abbreviated overview+userdoc presentation
- Demo: Toy examples for people to get started with.
- DONE dry-run opt for run.py, so it justs print the command.
** Env setup
More or less
#+begin_src bash
...
env RR_TEMPLATE_DIR=~/d/urap/libs/new-rr-templates
#+end_src
* Table template stuff
Header string:

Endpoint, params, filters
* Python API
- Basically a wrapper around webapp and build spectra
- Abstractly, if the error happens at the looking-for-data layer, we should ping the API, otherwise just report err to user
- Basic idea: Different layers
- Ideal: If we have to ping the server, we notify the user that we did this.
- TODO: API design. Basically, zcat_radec, zcat_target, spectra_radec, spectra_target
- HW: API design doc, with the URL \to func mapping established
